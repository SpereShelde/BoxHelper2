...
class TorrentCollector:
    ...
    def collect(self):
        ...
        for i in range(len(feed.entries)):
            torrent_list.append(Torrent(str(feed.entries[i].title), str(feed.entries[i].link), str(feed.entries[i].enclosures[0]["href"]), time.strftime("%Y-%m-%d %H:%M:%S", feed.entries[i].published_parsed)))
        indices = [] #存放匹配的title
