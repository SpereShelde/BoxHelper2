...
def rss():
    ...
    cursor.execute('SELECT title, size, promotions, detail_link, download_link, upload_time FROM torrents_collected where download_link != "" and promotions >= ? and size >= ? and size <= ? ORDER BY get_time DESC limit ?', (pro, low, high, num))
    ...
    for result in results:
        ...
        feed.append_item("%s [%s%s]" % (result[0], size_str, 'Free' if result[2]>=10 else ''), result[3], result[4], result[1])
